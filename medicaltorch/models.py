...
class NoPoolASPP(nn.Module):
    ...
    .. seealso::
        ...
        ArXiv link: http://arxiv.org/abs/1710.01269
    ...
    def __init__(self, drop_rate=0.4, bn_momentum=0.1):
        ...
        self.conv1a_bn = nn.BatchNorm2d(64, momentum=bn_momentum)
        self.conv1a_drop = nn.Dropout2d(drop_rate)
        self.conv1b = nn.Conv2d(64, 64, kernel_size=3, padding=1)
        self.conv1b_bn = nn.BatchNorm2d(64, momentum=bn_momentum)
        self.conv1b_drop = nn.Dropout2d(drop_rate)

        self.conv2a = nn.Conv2d(64, 64, kernel_size=3, padding=2, dilation=2)
        self.conv2a_bn = nn.BatchNorm2d(64, momentum=bn_momentum)
        self.conv2a_drop = nn.Dropout2d(drop_rate)
        self.conv2b = nn.Conv2d(64, 64, kernel_size=3, padding=2, dilation=2)
        self.conv2b_bn = nn.BatchNorm2d(64, momentum=bn_momentum)
        self.conv2b_drop = nn.Dropout2d(drop_rate)

        # Branch 1x1 convolution
        self.branch1a = nn.Conv2d(64, 64, kernel_size=3, padding=1)
        self.branch1a_bn = nn.BatchNorm2d(64, momentum=bn_momentum)
        self.branch1a_drop = nn.Dropout2d(drop_rate)
        self.branch1b = nn.Conv2d(64, 64, kernel_size=1)
        self.branch1b_bn = nn.BatchNorm2d(64, momentum=bn_momentum)
        self.branch1b_drop = nn.Dropout2d(drop_rate)

        # Branch for 3x3 rate 6
        self.branch2a = nn.Conv2d(64, 64, kernel_size=3, padding=6, dilation=6)
        self.branch2a_bn = nn.BatchNorm2d(64, momentum=bn_momentum)
        self.branch2a_drop = nn.Dropout2d(drop_rate)
        self.branch2b = nn.Conv2d(64, 64, kernel_size=3, padding=6, dilation=6)
        self.branch2b_bn = nn.BatchNorm2d(64, momentum=bn_momentum)
        self.branch2b_drop = nn.Dropout2d(drop_rate)

        # Branch for 3x3 rate 12
        self.branch3a = nn.Conv2d(64, 64, kernel_size=3, padding=12, dilation=12)
        self.branch3a_bn = nn.BatchNorm2d(64, momentum=bn_momentum)
        self.branch3a_drop = nn.Dropout2d(drop_rate)
        self.branch3b = nn.Conv2d(64, 64, kernel_size=3, padding=12, dilation=12)
        self.branch3b_bn = nn.BatchNorm2d(64, momentum=bn_momentum)
        self.branch3b_drop = nn.Dropout2d(drop_rate)

        # Branch for 3x3 rate 18
        self.branch4a = nn.Conv2d(64, 64, kernel_size=3, padding=18, dilation=18)
        self.branch4a_bn = nn.BatchNorm2d(64, momentum=bn_momentum)
        self.branch4a_drop = nn.Dropout2d(drop_rate)
        self.branch4b = nn.Conv2d(64, 64, kernel_size=3, padding=18, dilation=18)
        self.branch4b_bn = nn.BatchNorm2d(64, momentum=bn_momentum)
        self.branch4b_drop = nn.Dropout2d(drop_rate)

        # Branch for 3x3 rate 24
        self.branch5a = nn.Conv2d(64, 64, kernel_size=3, padding=24, dilation=24)
        self.branch5a_bn = nn.BatchNorm2d(64, momentum=bn_momentum)
        self.branch5a_drop = nn.Dropout2d(drop_rate)
        self.branch5b = nn.Conv2d(64, 64, kernel_size=3, padding=24, dilation=24)
        self.branch5b_bn = nn.BatchNorm2d(64, momentum=bn_momentum)
        self.branch5b_drop = nn.Dropout2d(drop_rate)

        self.concat_drop = nn.Dropout2d(drop_rate)
        self.concat_bn = nn.BatchNorm2d(6*64, momentum=bn_momentum)

        self.amort = nn.Conv2d(6*64, 128, kernel_size=1)
        self.amort_bn = nn.BatchNorm2d(128, momentum=bn_momentum)
        self.amort_drop = nn.Dropout2d(drop_rate)

        self.prediction = nn.Conv2d(128, 1, kernel_size=1)
...
class NoPoolASPPSmall(nn.Module):
    """
    .. image:: _static/img/nopool_aspp_arch.png
        :align: center
        :scale: 25%
